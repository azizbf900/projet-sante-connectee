...
public class FrontDetails implements Initializable {
    ...
    @FXML
    private Label produitInitialesLabel;
    @FXML
    ...
    @FXML
    private Label caracteristiquesLabel;
    @FXML
    private Label utilisationLabel;
    @FXML
    private Button btnAddToCart;
    @FXML
    private Button btnView3D;
    @FXML
    private HBox produitsSimilairesBox;
    ...
    private void afficherDetailsProduit() {
        ...
        // Afficher les initiales du produit
        produitInitialesLabel.setText(getInitiales(produit.getNom()));

        // Extraire les caractéristiques et l'utilisation depuis la description
        ...
        // Vérifier si le modèle 3D est disponible
        if (produit.getModel3D() == null || produit.getModel3D().isEmpty()) {
            btnView3D.setDisable(true);
        }
    }

    private void chargerProduitsSimilaires() {
        // Obtenir les produits de la même catégorie
        List<Produit> tousLesProduits = produitService.getAll();
        List<Produit> produitsSimilaires = tousLesProduits.stream()
        ...
        for (Produit p : produitsSimilaires) {
            ...
            produitsSimilairesBox.getChildren().add(card);
        }
    }
    ...
    private VBox creerCarteProduit(Produit p) {
        ...
        // Rectangle coloré avec initiales
        ...
        btnVoir.getStyleClass().add("btn-details");
    ...
    private void addToCart() {
        // Implémentation pour ajouter au panier
    ...
    private void view3DModel() {
        if (produit.getModel3D() != null && !produit.getModel3D().isEmpty()) {
            try {
                ...
                // Si vous avez un contrôleur pour afficher le modèle 3D
                // Model3DViewerController controller = loader.getController();
                // controller.setModelPath(produit.getModel3D());
    ...
    private void retourProduits() {
        try {
            ...
            // Utiliser la méthode correcte avec l'ID de catégorie du produit actuel
